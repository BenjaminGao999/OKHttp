...
public final class OkHttpManager {
    ...
    //okHttpManager修改为单例模式,okHttpManager实例化了，okHttpClient就实例化了。
     ...
     * @param callback 回调
     */
    public Response getAsync(final String url, final OkHttpUICallback.ResultCallback callback) throws IOException {
        final Request request = new Request.Builder().url(url).build();
        final Call call = mOkHttpClient.newCall(request);
     ...
     * @param json json格式的字符串
     */
    public Response postAsyncJson(String url, String json, OkHttpUICallback.ResultCallback callback) throws IOException {
        final RequestBody requestBody = RequestBody.create(JSON_TYPE, json);
        final Request request = new Request.Builder().url(url).post(requestBody).build();
    ...
    public void uploadAsync(String url, File[] files, String[] fileKeys,
        ...
        final RequestBody requestBody = buildMultipartFormRequestBody(files, fileKeys, params);
        final Request request = new Request.Builder().url(url).post(new ProgressBody.ProgressRequestBody(requestBody, uploadListener, mHandler)).build();
        mOkHttpClient.newCall(request).enqueue(new OkHttpThreadCallback(mHandler, uploadListener, false));
    }
                ...
                .addInterceptor(new Interceptor() {
                    ...
                    public Response intercept(Chain chain) throws IOException {
                        ...
                        return origin.newBuilder()
                                .body(new ProgressBody.ProgressResponseBody(origin.body(), downListener, mHandler))
                                .build();
                    }
                })
                .build();
        final Request request = new Request.Builder().url(url).build();
        downLoadClient.newCall(request).enqueue(new OkHttpThreadCallback(mHandler, downListener, true).setFile(downFile));
    }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (callback != null) {
                    ...
                    mHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            callback.onSuccess(object);//系统内部进行了一次类型强转
                        }
                    });
                }
            }
